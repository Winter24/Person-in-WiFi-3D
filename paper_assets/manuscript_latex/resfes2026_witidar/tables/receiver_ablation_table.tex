\begin{table}[!ht]
\caption{Per-receiver evaluation of Mamba-2 Flow (two steps) on the same held-out evaluation set as
Table~\ref{tab:main}. Receivers not in the subset are zero-masked
at the input of the pose-estimation pipeline (the model is not retrained).
The receiver masks follow the subset protocol reported in \cite{yan2024personwifi3d}. Units: mm; lower
is better.}
