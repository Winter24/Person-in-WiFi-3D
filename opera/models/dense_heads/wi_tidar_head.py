...
class WiTiDARHead(BaseModule):
    """
    Wi-FlowMamba Head: Architecture with Mamba Encoder, Draft MLP, and Refine Flow.
    """
    def __init__(self,
                 num_query=100,
                 embed_dims=256,
                 ...
                 # --- [REFACTORED 1]: Đọc cấu hình Mamba từ Config ---
                 mamba_cfg=dict(
                     num_layers=4,
                     d_state=16,
                     d_conv=4,
        ...
        if WiMambaEncoder is not None and mamba_cfg is not None:
            # Cho phép ghi đè type nếu có
            mamba_cfg_copy = mamba_cfg.copy()
            mamba_cfg_copy.pop('type', None) # Xóa 'type' nếu người dùng lỡ cấu hình
        ...
        # Refine Branch (Flow Matching)
        if VelocityMLP is not None:
            velocity_net = VelocityMLP(
                input_dim=3*num_keypoints, cond_dim=embed_dims, time_dim=64, hidden_dim=512, num_layers=3
            )
        ...
        for i in range(num_imgs):
            ...
            if len(pos_inds) > 0:
                ...
                if self.loss_limb is not None:
                    ...
                    loss_limb_val = self.loss_limb(pos_kpt_pred, pos_gt_kpts, limb_weight, avg_factor=num_pos)
                    losses_limb.append(loss_limb_val)
                ...
                if self.flow_model is not None:
                    ...
                    loss_flow = self.flow_model.get_train_loss(x_1=pos_gt_flat, condition=pos_cond, x_0=pos_draft_flat)
