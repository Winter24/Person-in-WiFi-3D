...
# Compatibility config for the original M0 baseline.  The old config declared
# a ResNet/ChannelMapper backbone, so we keep those modules to match checkpoint
# keys.  LegacyWifiPETR's forward path bypasses them and uses the top-level
# Linear(60, 256) projection, matching the original training code.
model = dict(
    type='opera.LegacyWifiPETR',
    backbone=dict(
        type='mmdet.ResNet',
        depth=50,
        num_stages=4,
...
lr_config = dict(policy='step', step=[90])
runner = dict(type='EpochBasedRunner', max_epochs=100)
find_unused_parameters = True
work_dir = './work_dirs/petr_wifi_legacy_linear'
auto_resume = False
gpu_ids = [0]
