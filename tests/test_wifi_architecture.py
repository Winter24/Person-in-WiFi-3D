...
class WifiArchitectureTests(unittest.TestCase):
    ...
    def test_wifi_input_adapter_has_spatial_mixing_before_temporal_fft(self):
        ...
        self.assertIn("x_permute = x_temp.permute(0, 2, 1)", source)
        self.assertIn("x_time = x_time.permute(0, 2, 1)", source)
        self.assertNotIn("nn.Conv1d(num_spatial, num_spatial * 2, kernel_size=1)", source)
    ...
    def test_wimamba_encoder_uses_factorized_spatiotemporal_blocks(self):
        ...
        self.assertIn("self.norm_s = nn.LayerNorm(num_spatial)", source)
        self.assertIn("self.spatial_mixer = nn.Sequential(", source)
        self.assertIn("nn.Linear(num_spatial, num_spatial * 2)", source)
        self.assertIn("nn.Linear(num_spatial * 2, num_spatial)", source)
        self.assertIn("self._init_weights()", source)
        ...
        self.assertIn("x_s = x.permute(0, 2, 3, 1)", source)
        self.assertIn("x_s = self.norm_s(x_s)", source)
        self.assertIn("x_s = self.spatial_mixer(x_s)", source)
        self.assertIn("x_s = self.drop_s(x_s)", source)
        self.assertIn("x_s = x_s.permute(0, 3, 1, 2)", source)
