...
class WifiArchitectureTests(unittest.TestCase):
    ...
    def test_wifi_input_adapter_has_spatial_mixing_before_temporal_fft(self):
        ...
        self.assertIn("self.spatial_norm = nn.LayerNorm(in_channels)", source)
        self.assertIn("self.spatial_mixer = nn.Sequential(", source)
        self.assertIn("nn.Conv1d(num_spatial, num_spatial * 2, kernel_size=1)", source)
        self.assertIn("nn.Conv1d(num_spatial * 2, num_spatial, kernel_size=1)", source)
        self.assertIn("self._init_weights()", source)
        self.assertIn("def _init_weights(self):", source)
        self.assertIn("nn.init.xavier_uniform_(self.spatial_mixer[0].weight)", source)
        self.assertIn("nn.init.constant_(self.spatial_mixer[0].bias, 0)", source)
        ...
        self.assertIn("nn.init.constant_(self.spatial_mixer[2].weight, 0)", source)
        self.assertIn("nn.init.constant_(self.spatial_mixer[2].bias, 0)", source)
        self.assertIn("x_spatial = x_grid.transpose(1, 2).contiguous().view(", source)
        ...
        self.assertIn("x_grid = x_spatial.view(B, self.seq_len, self.num_spatial, C)", source)
        self.assertIn("x_grid = x_grid.transpose(1, 2).contiguous()", source)
        self.assertIn("x_permute = x_temp.transpose(1, 2).contiguous()", source)
        self.assertIn("x_time = x_time.transpose(1, 2).contiguous()", source)
        self.assertNotIn("x_spatial = x_grid.permute(0, 2, 3, 1)", source)
        self.assertNotIn("nn.Linear(num_spatial, num_spatial * 2)", source)
    ...
    def test_wimamba_encoder_uses_factorized_spatiotemporal_blocks(self):
        ...
        self.assertIn("class FastFactorizedWiMambaBlock", source)
        ...
        self.assertIn("self.spatial_mixer = nn.Sequential(", source)
        self.assertIn("nn.Conv1d(num_spatial, num_spatial, kernel_size=1)", source)
        self.assertIn("x_t = x_t.reshape(B * S, T, C)", source)
        self.assertIn("x_s = x_s.transpose(1, 2).contiguous().view(B * T, S, C)", source)
        self.assertIn("x_s = self.spatial_mixer(x_s)", source)
        self.assertIn("x_s = self.drop_s(x_s).view(B, T, S, C).transpose(1, 2).contiguous()", source)
        self.assertIn("x = residual_t + x_t", source)
        self.assertIn("x = residual_s + x_s", source)
        self.assertIn("x = x.reshape(B, self.num_spatial, self.seq_len, C)", source)
        self.assertIn("x = x.permute(1, 0, 2).contiguous()", source)
        self.assertNotIn("torch.flip(", source)
        self.assertNotIn("self.mamba_s_fwd = Mamba(", source)
        self.assertNotIn("self.mamba_s_bwd = Mamba(", source)
