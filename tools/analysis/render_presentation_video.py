...
def render_presentation_video(records, model_asset, output_path, data_root=DEFAULT_DATA_ROOT,
                              ...
                              input_wave_smooth=9, input_wave_display='both'):
    if not records:
    ...
    try:
        ...
        for frame_index, record in enumerate(records):
            ...
            frame = _render_frame(
                ...
                input_wave_display=input_wave_display)
            if keep_frames_dir:
...
def build_arg_parser():
    ...
    parser.add_argument(
        '--input-wave-display',
        choices=['both', 'wave', 'activity'],
        default='both',
        help='CSI panel display mode: waveform only, activity curve only, or both.')
    parser.add_argument(
...
def main(argv: Optional[Sequence[str]] = None):
    ...
    render_presentation_video(
        ...
        input_wave_display=args.input_wave_display)
