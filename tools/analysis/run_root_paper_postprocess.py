#!/usr/bin/env python
"""Run postprocess for external paper runs under a single root directory.

This script scans direct subfolders in an external ``work_dirs/paper``
directory, treats each folder name as the experiment/model identifier,
then runs evaluation, benchmark, and CSV aggregation for each folder.
"""
...
DEFAULT_PAPER_DIR = REPO_ROOT.parent / 'root' / REPO_ROOT.name / 'work_dirs' / 'paper'
DEFAULT_OUTPUT_DIR = DEFAULT_PAPER_DIR
DEFAULT_EVAL_WORK_ROOT = REPO_ROOT / 'work_dirs' / 'paper_eval_root'


def parse_args(argv=None):
    parser = argparse.ArgumentParser(
        description='Postprocess external paper work dirs using folder names as experiment ids.')
    parser.add_argument(
        '--paper-dir',
        type=Path,
